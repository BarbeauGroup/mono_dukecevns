...
#include "mixtures.h"
  ...
  if (qfname != "none")
  {
    ...
    while (v != isotope_component.end())
    {
      ...
      outfilename = "out/diff_rates_quenched-" + std::string(jsonfile) + "-" + material + "-" + ffname + "-" + isoname + ".out";
      ...
      for (ie = 0; ie < iq; ie++)
      {
        if (Eee[is][ie] > maxeee && dNdEee[is][ie] > 0.)
        {
          maxeee = Eee[is][ie];
        }
    ...
    outfilename = "out/diff_rates_quenched-alliso-" + std::string(jsonfile) + "-" + material + "-" + ffname + ".out";
    ...
    if (gsname != "none")
    {
      ...
      outfilename = "out/diff_rates_smeared-alliso-" + std::string(jsonfile) + "-" + material + "-" + ffname + ".out";
    ...
    // The total qc 1.ribution (all components... could break it out by isotope)
    ...
    if (qcsmearing != "none")
    {
      ...
      std::cout << "Integral of qc 1., including zero bin: " << totinqc << std::endl;
      ...
      // Output the qc 1.ribution, applying efficiency if requested
      // (should not also have recoil or quenched efficiency)

      std::ofstream qcoutfile;
      outfilename = "out/diff_rates_qc-alliso-" + std::string(jsonfile) + "-" + material + "-" + ffname + ".out";
      ...
      std::cout << "Total qc events: " << totev << " unsmeared " << totevunsmeared << std::endl;
